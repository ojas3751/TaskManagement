...
| **1.3** | **2026-09-02** | **実装に合わせて記述を直した**（#103）。**仕様の変更ではなく、記録が実態に追いついていなかった分の補記。** ①`updated_at` の「更新時に現在時刻へ書き換える」は**未実装**（トリガも `ON UPDATE` も無い）であることを明記 ②**UUID の採番の説明が実装と逆**だったのを直した（サーバーが `randomUUID()` → **画面側が採番して API に渡す**）。あわせて `persist()` を使う理由を追記 ③4章のリスト再採番の行数を **20行 → 13行**（追加10＋デフォルト3）に修正。API設計 v2.1 の上限変更が反映漏れだった ④cards の `cards_due_time_needs_date` CHECK と、**インデックス2本**を表に追記 |
...
| created_at | 不可 | `now()` | |
| updated_at | 不可 | `now()` | **INSERT 時に `now()` が入るだけ。更新時の自動更新は未実装** |

> **`updated_at` は行を更新しても変わらない。** トリガも `ON UPDATE` も置いていない（PostgreSQL には MySQL の `ON UPDATE CURRENT_TIMESTAMP` に当たる仕組みが無く、トリガを書く必要がある）。エンティティ側も `insertable = false, updatable = false` にしてあり、アプリからも書かない。**API では返さず、画面でも使っていない**（[API設計](api.md) 2.6）ため、現状は困っていない。必要になったら V3 でトリガを追加する。
...
| has_due_time | 不可 | `false` | 時分が指定されたか。**`due_at` が NULL のとき true にはできない**（CHECK `cards_due_time_needs_date`） |
...
| 名前 | 対象 | 用途 |
| --- | --- | --- |
| `lists_board_position_idx` | `lists (board_id, position)` | 並び順つきの取得（`GET /api/board`） |
| `cards_list_position_idx` | `cards (list_id, position)` | 同上 |

> **`(list_id, position)` に UNIQUE は付けない。** 一括再採番の途中で必ず値が重複するため（4章）。
...
- **UUID は画面側で採番し、API に渡す。** サーバーは受け取った id をそのまま永続化する（`POST /api/lists` と `POST /api/cards` の `id`）。`@GeneratedValue` も DB 側の `gen_random_uuid()` も使わない。**サーバーが振り直すと、応答を待たずに描いたカードと DB の id が食い違う**ため。上記の楽観的更新は「サーバーに問い合わせる前に id が決まっている」ことで成立している
- **新規追加では `save()` ではなく `persist()` を使う。** id をアプリ側で採番している以上、`save()` からは「id を持つ＝既存の行かもしれない」と見えて `merge` に倒れ、INSERT の前に不要な SELECT が1回走る
...
| リストの追加・並び替え・削除 | **`lists` のみ**再採番（**最大13行** = 追加できる10件＋デフォルト3列）。タスクは触らない |
