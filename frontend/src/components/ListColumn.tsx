    ...
    >
      ...
      {/* 横はスクロールさせない（overflow-x-hidden）。カードは列に収まる幅で固定して
          ...
          **縦のスクロールバーは細くする。** 既定の幅（15px 前後）はカードの右余白より
          ...
          無いので、プロパティを直接指定する */}
      {/* **上下に余白を取る。理由は2つあり、どちらも「カードの外側に描かれるもの」。**

          1. 挿入位置の線（InsertionLine）は、並びを動かさないために負のマージンで
             場所を取らずに描いている。そのぶん、線が先頭や末尾に出たときはこの領域から
             1px はみ出す。**はみ出せば縦のスクロールバーが出る。** 線が半分しか
             見えないのも同じ原因
          2. **フォーカスの輪郭線**（#113 / C-10。`index.css` の :focus-visible）。
             2px の線を 2px 外側に描くので、**先頭のカードでは上辺が領域の外に出て
             切れる。** 見えなければフォーカスを示す役に立たない

          **必要なのは 4px。** 1px（py-px）では 1. しか賄えない。

          **左右も同じだけ要るが、そちらは負のマージンで打ち消す**（px-1 -mx-1）。
          横に余白を足すとカードが 8px 細くなり、**カードを縮めずに列を広げた判断（#70）を
          裏切る。** 列の内側の余白（p-2.5 ＝ 10px）へ食い込ませれば、カードの幅は
          変わらないまま輪郭だけが収まる。**縦で同じ手を使わないのは、`gap-2` の見た目まで
          詰めてしまうため** */}
      ...
      <div
        ...
        className="-mx-1 flex min-h-0 flex-col gap-2 overflow-x-hidden overflow-y-auto px-1 py-1 [scrollbar-width:thin]"
      >
